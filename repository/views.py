...
def index(request):
    ...
    return render_to_response('index.html', {'nentries':nentries, 'nRNAs':nRNAs, 'nconstructs':nconstructs,\
                              'ndatapoints':ndatapoints, 'news':news}, context_instance=RequestContext(request))
...
def detail(request, rmdb_id):
	...
	for c in constructs:
        ...
        raise Http404
    return render_to_response('detail.html', {'codebase':get_codebase(request), 'entry':entry, 'constructs':constructs, 'publication':entry.publication,\
           'comments':comments, 'data_annotations_exist':data_annotations_exist, 'maxlen_flag':maxlen_flag}, context_instance=RequestContext(request))
...
def validate(request):
	...
	if request.POST['type'] == 'rdat':
		...
		if os.exists(path):
		    ...
		    form = ValidateForm()
		    messages = []
		    return render_to_response('validate.html', {'form':ValidateForm(), 'valerrors':errors,'messages':[]},\
...
def advanced_search(request):
	...
	try:
		...
		if len(request.POST['structure']) != len(request.POST['sequence']) and \
		   len(request.POST['structure']) > 0 and len(request.POST['sequence']) > 0:
