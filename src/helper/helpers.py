...
def upload_file(uploadfile, publication, description, authors, type, pubmed_id, filetype='rdat'):
    ...
    if proceed:
        ...
        if not os.path.exists('%s%s' % (PATH.DATA_DIR['FILE_DIR'], entry.rmdb_id)):
            os.mkdir('%s%s' % (PATH.DATA_DIR['FILE_DIR'], entry.rmdb_id))
        rdatname = '%s%s/%s_%s.rdat' % (PATH.DATA_DIR['FILE_DIR'], entry.rmdb_id, entry.rmdb_id, entry.version)
        os.popen('mv /tmp/%s %s' % (filename, rdatname))
        os.popen('cp %s %s%s/%s.rdat' % (rdatname, PATH.DATA_DIR['FILE_DIR'], entry.rmdb_id, entry.rmdb_id))
        os.popen('cp %s %s%s/%s_synced.rdat' % (rdatname, PATH.DATA_DIR['FILE_DIR'], entry.rmdb_id, entry.rmdb_id))
