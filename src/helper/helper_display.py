...
def dump_json_heatmap(construct, entry_type, maxlen):
    ...
    try:
        ...
        return None
    return {'data':data_matrix, 'peak_max':data_max, 'peak_min':round(data_min, 3), 'peak_mean':round(data_mean, 3), 'peak_sd':round(data_sd, 3), 'row_lim':row_limits, 'x_labels':x_labels, 'y_labels':y_labels, 'precalc_structures':precalc_structures}
...
def prepare_json_data(entry):
    ...
    for c in constructs:
        ...
        open(PATH.DATA_DIR['RDAT_FILE_DIR'] + '/' + entry.rmdb_id + '/data_heatmap.json', 'w').write(simplejson.dumps(json_tmp, sort_keys=True))
...
def make_json_for_rdat(rmdb_id):
    ...
    if entry.pdb != None and len(entry.pdb.strip()) > 0:
