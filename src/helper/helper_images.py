...
def generate_varna_thumbnails(entry):
    try:
        ...
        for c in constructs:
            path = '%s%s' % (PATH.DATA_DIR['THUMB_DIR'], c.id)
            fname = path
            ...
            if is_structure and is_SS and (not is_large) and (not is_eterna):
                ...
                os.popen('convert %s%s/reactivity_crisp.png %s.gif' % (PATH.DATA_DIR['IMG_DIR'], c.id, fname))
                os.popen('convert %s%s/reactivity_blur.png %s_blur.gif' % (PATH.DATA_DIR['IMG_DIR'], c.id, fname))
...
def generate_images(construct_model, construct_section, entry_type, engine='matplotlib'):
    ...
    dir = PATH.DATA_DIR['IMG_DIR'] + '%s/' % construct_model.id
