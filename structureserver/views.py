...
def index(request):
	...
	try:
	    ...
	    messages = []
	    valerrors = []
	    if ('rmdbid' in request.POST and request.POST['rmdbid'])\
		 or 'rdatfile' in request.FILES:
		temperature = 37
		offset_seqpos = []
		rdatfile = RDATFile()
		...
		for cname in rdatfile.constructs:
			...
			valerrors.append('Warning: Construct %s in RDAT file has no expected secondary structure or has a structure that does not match SEQPOS,\
			 filling out a trivial structure instead' % c.name)
		...
		return render_to_response('structureserver.html', {'form':form, 'rdatloaded':True, 'valerrors':valerrors})
...
#======  End RDAT file and RMDB entry parsing ======
		...
		return render_to_response('view_structures.html', {'panels':[], 'messages':messages,\
					  'bppmimg':'', 'ncols':0, 'nrows':0}, context_instance=RequestContext(request))
...
#TODO This validation should be done client side
	    ...
	    return render_to_response('view_structures.html', {'panels':panels, 'messages':messages,\
				      'ncols':ncols, 'nrows':nrows, 'form':visform}, context_instance=RequestContext(request))
	except IndexError:
	    return render_to_response('structureserver.html', {'form':PredictionForm(), 'rdatloaded':False, 'other_errors':['Invalid input. Please check your inputs and try again.']})

    else:
	form = PredictionForm()
	return render_to_response('structureserver.html', {'form':form, 'rdatloaded':False, 'other_errors':[]}, context_instance=RequestContext(request))
...
def render_to_varna(sequences, structures, modifiers, titles, mapping_data, base_annotations, refstruct):
    ...
    for i in range(len(sequences)):
	    ...
	    v.width = 400
	    v.height = 600
	    panels.append(v.render(base_annotations=base_annotations[i:i+nelems], annotation_by_helix=True, annotation_def_val='0.0%', \
			  helix_function=(lambda x, y: str(max(float(str(x).strip('%')), float(str(y).strip('%')))) + '%'), reference_structure=refstruct))
...
def viewstructures(request):
	...
	return render_to_response('view_structures.html', {'panels':panels, 'messages':messages,\
				  'ncols':ncols, 'nrows':nrows, 'form':form}, context_instance=RequestContext(request))
