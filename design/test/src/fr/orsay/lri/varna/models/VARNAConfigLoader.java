...
public class VARNAConfigLoader {
	...
	public static String offsetOpt = "offset";
	...
	public static String[][] getParameterInfo() 
	{
		String[][] info = {
			...
			{ offsetOpt, "int", "Offset of base-numbering" },
	...
	private void retrieveParametersValues() throws ExceptionParameterError,
		...
		for (int i = 0; i < _numColumns; i++) {
			for (int j = 0; j < _numRows; j++) {
				...
				try {
					_offset = Integer.parseInt(_optionProducer
							.getParameterValue(offsetOpt + n, ""
									+ _offset));
				} catch (NumberFormatException e) {
					throw new ExceptionParameterError(
							e.getMessage(),
							"'"
									+ _optionProducer.getParameterValue(
											offsetOpt + n, ""
													+ _offset)
									+ "' is not a valid integer value for the offset!");
				}
	...
	private void applyValues(String n)
		...
		_mainSurface.setOffset(_offset);
